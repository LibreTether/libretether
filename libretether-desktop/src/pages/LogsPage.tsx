...
/** Sources produced by the controller's own buffer — everything else is an agent's
 *  name. Relay lines are included: in relay mode the controller polls the relay's
 *  server log in the background and folds it into its own buffer (source "relay"),
 *  so it arrives like any other controller line. Used so a refresh of the controller
 *  buffer replaces these in place without duplicating them, while leaving on-demand
 *  agent logs untouched. */
const CONTROLLER_SOURCES = new Set(["controller", "tunnel", "rdp", "ssh", "session", "relay"])
