...
import { ArrowRight, Bug, Pencil, Plus, Trash2 } from "lucide-react"
...
export function ControllerSelect({ onConnected }: { onConnected: (a: ActiveInfo) => void }) {
	...
	return (
		<div className="flex h-screen flex-col overflow-hidden">
			...
			<div className="flex min-h-0 flex-1 items-center justify-center overflow-y-auto p-6">
				<div className="flex w-full max-w-lg flex-col gap-7">
					<div className="flex flex-col items-center gap-3 text-center">
						<img alt="" className="h-14 w-14 rounded-2xl" src="/libretether.png" />
						<div>
							<h1 className="font-display text-2xl font-bold tracking-tight text-text">LibreTether</h1>
							<p className="mt-1.5 text-sm text-muted">Reach your machines over your own private mesh.</p>
						</div>
					</div>

					<div className="flex flex-col gap-2.5">
						<div className="eyebrow px-1">Controllers</div>
						{loading ? (
							...
							<>
								{controllers.map((c) => {
									...
									return (
										<div
											className="card group flex items-center gap-3.5 p-4 transition hover:border-primary/40"
											key={c.id}
										>
								...
								<Button
									className="justify-center border border-dashed border-border-strong py-3 hover:border-primary/50"
			...
			<footer className="flex shrink-0 items-center justify-center gap-2 pb-4 font-mono text-xs text-subtle">
